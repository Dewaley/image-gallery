...
function App() {
  ...
  const handlePageClick = (data) => {
    const url = `https://pixabay.com/api/?key=25912123-f6cf2c45649c1add9c51bacc6&q=${tag}&image_type=photo&per_page=30&page=${data.selected+1}`;
    console.log(url)
    fetch(url)
      .then((res) => res.json())
  ...
  return (
    <div className='container mx-auto'>
      ...
      {pageCount > 1 && results !== [] &&(<ReactPaginate
        previousLabel={'<<'}
        nextLabel={'>>'}
        breakLabel={'...'}
        pageCount={pageCount}
